...
VARS='$NAMESPACE $SITE_HOST $ENTRYPOINT $SITE_ENV $APP_DIR $WEB_IMAGE $PIPELINE_IMAGE $DAILY_SCHEDULE $WEEKLY_SCHEDULE $REFRESH_GAMEDAY_SCHEDULE $REFRESH_HOURLY_SCHEDULE $NEWSROOM_SCHEDULE $NEWSROOM_SUSPEND $NEWSROOM_AUTOPUBLISH $SUPPORT_EMAIL $ADS_ENABLED $ADS_TRUST_CLIENT_REWARDS $PRO_ENABLED $ADMOB_BANNER_UNIT $ADMOB_NATIVE_UNIT $ADMOB_REWARDED_UNIT'
