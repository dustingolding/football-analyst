...
CREATE TABLE IF NOT EXISTS push_devices (
    install_id      TEXT        PRIMARY KEY,  -- random per install, generated by the app
    apns_token      TEXT        NOT NULL UNIQUE,
    environment     TEXT        NOT NULL,     -- APNs server: sandbox (Xcode builds) or production
    bundle_id       TEXT        NOT NULL,     -- APNs topic
    timezone        TEXT,
    alert_kickoff   BOOLEAN     NOT NULL DEFAULT true,
    alert_scoring   BOOLEAN     NOT NULL DEFAULT true,
    alert_final     BOOLEAN     NOT NULL DEFAULT true,
    api_key_prefix  TEXT,                     -- which API key registered it
    disabled_at     TIMESTAMPTZ,              -- set when APNs says the token is dead
    last_error      TEXT,
    created_at      TIMESTAMPTZ NOT NULL DEFAULT now(),
    updated_at      TIMESTAMPTZ NOT NULL DEFAULT now()
);
...
-- The last state and score notify.py saw for each live game; alerts fire on changes from it.
...
-- One row per alert. Inserting first (ON CONFLICT DO NOTHING) is what keeps a restart or retry
-- from sending the same alert twice.
CREATE TABLE IF NOT EXISTS push_events (
    league      TEXT        NOT NULL,
    game_id     TEXT        NOT NULL,
    kind        TEXT        NOT NULL,         -- kickoff, score, final, test
    detail      TEXT        NOT NULL,         -- e.g. the score "14-7", so each change is its own event
    title       TEXT,
    body        TEXT,
    sent        INTEGER     NOT NULL DEFAULT 0,
    failed      INTEGER     NOT NULL DEFAULT 0,
    created_at  TIMESTAMPTZ NOT NULL DEFAULT now(),
    PRIMARY KEY (league, game_id, kind, detail)
);
